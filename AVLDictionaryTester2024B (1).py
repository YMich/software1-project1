...
class AVLTreeTester2022(unittest.TestCase):
    ...
    def test_delete_operations_simple_1(self):
        ...
        #   2
        # 1   3
        self.assertEqual(self.tree.delete(self.tree.select(1)), 0, "FAIL - deleting '1' should cost 0 re-balancing op")
        self.assertEqual(self.tree.delete(self.tree.select(2)), 1, "FAIL - deleting '3' should cost 1 re-balancing op")
        self.assertEqual(self.tree.delete(self.tree.select(1)), 0, "FAIL - deleting '2' should cost 0 re-balancing op")
        self.add_points(2)
    ...
    def test_retrieve_linear_insertion(self):
        for i in range(100):
            self.tree.insert(i, str(i))
        for i in range(100):
            self.assertEqual(self.tree.select(i+1).value, str(i), "FAIL - iteration {}".format(i))
        self.add_points(1)
    ...
    def test_rank(self):
        ...
        avl_tree = self.create_tree(keys, random_order=True)

        for i in keys:
            self.assertEqual(i , avl_tree.rank(avl_tree.search(i)))

        self.add_points(3)

    def test_big_rank(self):
        keys = [i for i in range(1, 500)]
        avl_tree = self.create_tree(keys, random_order=True)
